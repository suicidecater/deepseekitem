...
def _user_to_dict(u, role):
    ...
    if role == 'student':
        ...
        d['coach_id'] = u.coach_id
        if u.coach_id and u.coach:
            d['coach_name'] = u.coach.school_name or f'教练 #{u.coach.id}'
            d['coach_phone'] = u.coach.school_phone or ''
    elif role == 'coach':
    ...
    if role == 'student':
        ...
        if 'coach_id' in data:
            ...
            if coach_id is not None:
                ...
                if not coach:
                    return error(40001, '教练不存在')
                if coach.status != 1:
                    return error(40001, '教练已离职或禁用，无法分配')
