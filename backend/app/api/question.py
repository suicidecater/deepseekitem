...
from app.models.question import ErrorQuestion, SpecialTrainingSession
...
def submit_special_training():
    ...
    for a in answers:
        ...
        if q:
            ...
            # 写入答题记录（用于雷达图动态计算）
            QuestionService._record_answer(g.user_id, qid, qid_source_map.get(qid, 'subject1'), q.category, is_correct)
            answer_map[str(qid)] = {
                'userAnswer': user_ans,
    ...
    # 写入专项训练提交记录
    try:
        session = SpecialTrainingSession(
            student_id=g.user_id,
            category=category,
        ...
        current_app.logger.error(f'写入专项训练提交记录失败: {e}')
...
def check_sequential_answer():
    ...
    ok, msg, result = QuestionService.check_sequential_answer(
        question_id=question_id, source=source, user_answer=user_answer, student_id=g.user_id
    )
    if not ok:
        return error(400, msg)
    return success(result, message=msg)
