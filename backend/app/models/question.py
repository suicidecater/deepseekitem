...
class QuestionRecord(BaseModel):
    ...
    student_id = db.Column(db.BigInteger, db.ForeignKey('student.id'), nullable=False, index=True, comment='学员ID')
    question_id = db.Column(db.BigInteger, nullable=False, index=True, comment='题目ID')
    source = db.Column(db.String(20), nullable=False, default='subject1', comment='题库来源: subject1/subject4/professional')
    category = db.Column(db.String(20), nullable=True, comment='分类: 交通标志/交通法规/安全常识/驾驶理论')
    is_correct = db.Column(db.SmallInteger, default=0, comment='是否正确: 1正确/0错误')
    answer_time = db.Column(db.DateTime, default=datetime.now, comment='答题时间')

    __table_args__ = (
        db.Index('idx_record_student_category', 'student_id', 'category'),
        db.Index('idx_record_student_question', 'student_id', 'question_id', 'source'),
    )


# 专项训练提交记录表
class SpecialTrainingSession(BaseModel):
    __tablename__ = 'special_training_session'

    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True, comment='记录ID')
    student_id = db.Column(db.BigInteger, db.ForeignKey('student.id'), nullable=False, index=True, comment='学员ID')
    category = db.Column(db.String(20), nullable=False, comment='分类: 交通标志/交通法规/安全常识/驾驶理论')
    question_count = db.Column(db.Integer, default=0, comment='题目数量')
    correct_count = db.Column(db.Integer, default=0, comment='正确题数')
    correct_rate = db.Column(db.Float, default=0.0, comment='正确率')
    submit_time = db.Column(db.DateTime, default=datetime.now, comment='提交时间')

    __table_args__ = (
        db.Index('idx_session_student_time', 'student_id', 'submit_time'),
    )
