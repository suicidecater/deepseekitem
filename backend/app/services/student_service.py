...
from app.models.question import Subject1Question, ErrorQuestion, Subject4Question, ProfessionalQuestion, QuestionRecord
...
class StudentService:
    ...
    # 雷达图缓存 key 前缀
    ABILITY_RADAR_CACHE_PREFIX = 'ability_radar:'
    ABILITY_RADAR_CACHE_TTL = 3600  # 缓存1小时

    @classmethod
    def get_ability_radar(cls, student_id):
        """
        获取学员能力雷达图四维度动态分数
        统计逻辑：遍历 question_record 表，按 category 分组加权计算
        无答题记录时四个维度默认 50 分
        """
        # 1. 尝试从 Redis 缓存读取
        cache_key = f'{cls.ABILITY_RADAR_CACHE_PREFIX}{student_id}'
        try:
            from app.extensions import redis_store
            cached = redis_store.get(cache_key) if redis_store else None
            if cached:
                import json
        ...
        dim_scores = {'交通标志': 50, '交通法规': 50, '安全常识': 50, '驾驶理论': 50}

        # 统计各维度答题情况
        stats = db.session.query(
            QuestionRecord.category,
            sqla_func.count(QuestionRecord.id).label('total'),
            sqla_func.sum(QuestionRecord.is_correct).label('correct')
        ).filter(
            QuestionRecord.student_id == student_id,
            QuestionRecord.category.in_(['交通标志', '交通法规', '安全常识', '驾驶理论'])
        ).group_by(QuestionRecord.category).all()

        if stats:
            for stat in stats:
                cat = stat[0]
                total = int(stat[1] or 0)
                correct = int(stat[2] or 0)
                if total > 0 and cat in dim_scores:
                    # 基础正确率 × 100
                    base_score = round((correct / total) * 100)

                    # 练习次数小幅加成（多次练习同一题型小幅提升，最多+5分）
                    practice_bonus = min(5, total * 0.1)

                    # 最终得分：基础正确率分 + 练习加成，上限100
                    score = min(100, base_score + int(practice_bonus))
                    dim_scores[cat] = score

        result = {
            'traffic_sign': dim_scores['交通标志'],
            'traffic_law': dim_scores['交通法规'],
            'safety_common': dim_scores['安全常识'],
            'driving_theory': dim_scores['驾驶理论'],
        }
        ...
        try:
            ...
            if redis_store:
                import json
                redis_store.setex(cache_key, cls.ABILITY_RADAR_CACHE_TTL, json.dumps(result))
        except Exception:
            pass
    ...
    @classmethod
    def invalidate_ability_radar_cache(cls, student_id):
        """清除指定学员的能力雷达图缓存（答题/交卷后调用）"""
        try:
            from app.extensions import redis_store
            if redis_store:
                cache_key = f'{cls.ABILITY_RADAR_CACHE_PREFIX}{student_id}'
    ...
    @classmethod
    def get_home_stat(cls, student_id):
        """
        获取学员首页统计数据（动态计算）
        返回: {
            student_name: 学员真实姓名,
            continue_study_days: 连续学习天数,
            today_study_minutes: 今日学习分钟数,
            total_done_questions: 累计完成做题总数(去重)
        }
        """
        ...
        # 1. 学员姓名
        student_name = student.name or '学员'

        # 2. 连续学习天数（基于 QuestionRecord 答题记录）
        continue_study_days = cls._calc_continue_study_days(student_id)

        # 3. 今日学习分钟数
        today_study_minutes = cls._calc_today_study_minutes(student_id)

        # 4. 累计完成做题总数（去重）
        total_done_questions = cls._calc_total_done_questions(student_id)

        return {
            'student_name': student_name,
            'continue_study_days': continue_study_days,
            'today_study_minutes': today_study_minutes,
            'total_done_questions': total_done_questions,
        }

    @classmethod
    def _calc_continue_study_days(cls, student_id):
        """
        计算连续学习天数
        逻辑：从今日往前倒推，每日存在答题记录则累计，断一天清零重新计算
        """
        records = db.session.query(
            func.date(QuestionRecord.answer_time).label('day')
        ).filter(
            QuestionRecord.student_id == student_id
        ).distinct().order_by(func.date(QuestionRecord.answer_time).desc()).all()
    ...
    @classmethod
    def _calc_today_study_minutes(cls, student_id):
        """
        计算今日学习分钟数
        逻辑：统计当天所有 QuestionRecord 答题数量，按每题约1分钟估算
        """
        today_start = datetime.combine(date.today(), datetime.min.time())
        today_end = datetime.combine(date.today(), datetime.max.time())

        # 统计今日答题数量
        today_questions = QuestionRecord.query.filter(
            QuestionRecord.student_id == student_id,
            QuestionRecord.answer_time >= today_start,
            QuestionRecord.answer_time <= today_end
        ).count()

        # 估算：每题约1分钟
        return max(0, today_questions)

    @classmethod
    def _calc_total_done_questions(cls, student_id):
        """
        计算累计完成做题总数（去重）
        逻辑：统计 QuestionRecord 中不重复的 (question_id, source) 数量
        """
        total = db.session.query(
            func.count(func.distinct(
                func.concat(QuestionRecord.question_id, '_', QuestionRecord.source)
            ))
        ).filter(
            QuestionRecord.student_id == student_id
        ).scalar() or 0
