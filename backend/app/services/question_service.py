...
from app.models.question import (
    ...
    ErrorQuestion, StudentQuestionMemory, QuestionRecord, QuestionService as QS
)
...
class QuestionService:
    ...
    def answer_practice(student_id, practice_id, question_id, user_answer, source=None):
        ...
        QuestionService._record_answer(student_id, question_id, source, question.category, is_correct)
    ...
    def submit_exam(student_id, exam_id, total_time=None, source=None):
        ...
        for qid in qids:
            ...
            is_correct = QuestionService._check_answer(ua, q.correct_answer, q.question_type)
            if is_correct:
                correct += 1
                dim_correct[dim] += 1
            else:
            ...
            # 写入答题记录（用于雷达图动态计算）
            QuestionService._record_answer(student_id, qid, source, q.category, is_correct)
    ...
    def review_error_question(student_id, error_id, user_answer=None):
        ...
        if user_answer is not None and user_answer != '':
            ...
            # 写入答题记录
            QuestionService._record_answer(student_id, q.id, eq.source, q.category, is_correct)
            if is_correct:
                # 做对直接删除错题记录
    ...
    @staticmethod
    def check_sequential_answer(question_id, source='subject1', user_answer='', student_id=None):
        ...
        # 写入答题记录
        if student_id:
            QuestionService._record_answer(student_id, question_id, source, q.category, is_correct)
        return True, 'success', {
            'correct': is_correct,
    ...
    @staticmethod
    def _record_answer(student_id, question_id, source, category, is_correct):
        """记录每次答题到 question_record 表，并清除雷达图缓存"""
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f'写入答题记录失败: {e}')

        # 清除能力雷达图缓存（下次请求时重新计算）
        try:
            from app.services.student_service import StudentService
            StudentService.invalidate_ability_radar_cache(student_id)
        except Exception:
            pass
