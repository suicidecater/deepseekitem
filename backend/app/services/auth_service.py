...
class AuthService:
    ...
    def _format_user_response(user, user_type):
        ...
        if user_type == 'student':
            ...
            result['coach_id'] = data.get('coach_id')  # 教练ID，用于对话等场景
            # 附带教练信息
            if hasattr(user, 'coach') and user.coach:
                result['coach_name'] = user.coach.school_name or f"教练 #{user.coach.id}"
                result['coach_phone'] = user.coach.school_phone or ''
            # 各方向的测评状态
            status_map = {}
            for d_subj in [1, 4, 5]:
        ...
        if user_type == 'coach':
            result['school_name'] = data.get('school_name', '')
        return result
