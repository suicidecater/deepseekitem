...
// src/views/auth/RegisterStudent.vue - 学员注册
...
function validate(): boolean {
  ...
  if (!form.name.trim() || form.name.trim().length < 2) {
    errors.name = '姓名至少2个字符'; valid = false
  }
      ...
      <form class="form-body" @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <div class="password-wrapper">
            ...
            <button type="button" class="password-toggle" @click="confirmVisible = !confirmVisible" tabindex="-1">
              {{ confirmVisible ? '🙈' : '👁' }}
            </button>
          </div>
        ...
        <div class="form-group">
          ...
          <input v-model="form.name" type="text" placeholder="请输入真实姓名" class="form-input" :class="{ error: errors.name }" />
        ...
        <!-- 报考车型（可选） -->
        <div class="form-group">
          <label class="form-label">报考车型（可选）</label>
          <select v-model="form.carType" class="form-input">
