...
import {
  coachGetNotifications,
  coachReadNotification,
  coachReadAllNotifications,
...
async function fetchList() {
  loading.value = true
  try {
    const res = await coachGetNotifications({
      page: page.value,
...
async function handleClick(item: NotificationItem) {
  ...
  if (item.is_read === 0) {
    try {
      await coachReadNotification(item.id)
      item.is_read = 1
...
async function handleReadAll() {
  try {
    await coachReadAllNotifications()
    ...
    console.error('[教练端-全部标记已读] 失败:', err)
